...
export const CoachSuggestionCard = ({ level = 'Mild' }) => {
  const PACKAGES = {
    Mild: {
      ...
      nicotine: '5 mg',
      price: 35000,
      link: 'https://www.who.int/health-topics/tobacco#tab=tab_1',
    },
    Moderate: {
      name: 'Moderate Pack B',
      nicotine: '10 mg',
      price: 50000,
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-6 flex flex-col gap-3">
      <h3 className="font-semibold text-emerald-700">
        🚬 Alternative Cigarette Package
      </h3>

      <p className="text-sm text-gray-700">
        Suggested: <b>{pack.name}</b> — {pack.nicotine} Nicotine
      </p>
      <p className="text-sm text-gray-700">
...
export const CoachFeedbackCard = ({
  ...
  const [tempRating, setTemp] = useState(5);           // ⭐ mặc định 5 sao
  const [tempComment, setCom] = useState('');          // 📝 mặc định trống

  const openModal = () => {
    setTemp(savedRating || 5);                         // nếu có feedback cũ thì dùng
    setCom(savedComment || '');
    setOpen(true);
  };
      ...
      <div className="bg-white rounded-2xl shadow-xl p-6 space-y-4">
        ...
        {savedRating > 0 && (
          <div className="space-y-1 pt-3 border-t">
            ...
            {savedComment && (
              <p className="text-sm text-gray-600 italic">“{savedComment}”</p>
            )}
          </div>
        )}
